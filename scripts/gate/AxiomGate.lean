/-
  AxiomGate.lean — full-environment axiom allowlist gate for lopt-entropy-formal.

  Walks EVERY non-internal declaration whose defining module is a first-party
  project module (module name begins with `LoptEntropy`), runs `collectAxioms`
  on each, and QUARANTINES any declaration whose transitive axiom set is not a
...
private def isFirstPartyModule (m : Name) : Bool :=
  m == `LoptEntropy || (`LoptEntropy).isPrefixOf m
